...
def doit_traiter(spec_actuel, derniere_signature):
    """
    Retourne (a_traiter, signature_actuelle).

    Signature basée sur spec, jamais sur status
    (qui change à chaque patch du handler lui-même).
    """
...
def get_deployment_replicas(
    ...
    try:
        ...
        logging.error(
            f"Lecture Deployment {namespace}/{name} echouee: {exc}"
        )
...
def scale_deployment(
    ...
    try:
        ...
        logging.error(
            f"Scale de {namespace}/{name} a {replicas} replicas echoue: {exc}"
        )
...
def evaluate_and_act(namespace, spec, status, patch, logger):
    ...
    cycles_threshold = spec.get(
        'consecutiveCyclesThreshold',
        CONSECUTIVE_CYCLES_DEFAULT
    )
    ...
    action_state = status.get(
        'actionState',
        'NORMAL'
    )

    consecutive = status.get(
    ...
    if decision == "DEPASSEMENT":
        ...
        if consecutive < cycles_threshold:
            ...
            # Garde-fou anti-flapping :
            # dépassement pas encore confirmé.
            logger.warning(
            ...
            if not target or min_replicas is None:
                ...
                replicas_avant = get_deployment_replicas(
                    namespace,
                    target
                )
                ...
                if replicas_avant is None:
                    ...
                    logger.error(
                        f"Deployment {target} introuvable, "
                        f"action corrective annulee."
                    )

                else:

                    succes = scale_deployment(
                        namespace,
                        target,
                        min_replicas
                    )
                    ...
                    if succes:
                        ...
                        historique = (
                            status.get('correctiveActionTaken')
                            or []
                        )

                        patch.status['correctiveActionTaken'] = (
                            historique + [entree]
                        )

                        # Etat pré-action stocké pour le rollback automatique.
                        patch.status['preActionState'] = {
                            'replicas': replicas_avant
                        }
            ...
            logger.info(
                f"DEPASSEMENT persistant, action deja active sur {target}."
            )

    # -----------------------------------------------------------------------
    # OK : coût revenu sous le seuil
    # -----------------------------------------------------------------------
        ...
        if action_state == 'ACTION_TAKEN':
            ...
            if target and replicas_a_restaurer is not None:
                ...
                replicas_actuels = get_deployment_replicas(
                    namespace,
                    target
                )
                ...
                if replicas_actuels is None:
                    ...
                    # Le rollback est déjà effectif.
                    #
                    # Exemple :
                    # ArgoCD peut avoir remis le Deployment dans son état
                    # Git avant que l'opérateur n'effectue lui-même le rollback.
                    #
                    # Dans ce cas, inutile de modifier à nouveau le Deployment.

                    entree = entry_rollback(
                        target,
                        replicas_a_restaurer
                    )
                    ...
                    logger.info(
                        f"Rollback deja effectif: {target} est deja a "
                        f"{replicas_a_restaurer} replicas."
                    )

                elif replicas_actuels == min_replicas:

                    # Le Deployment est toujours dans l'état post-action.
                    #
                    # On peut donc effectuer le rollback normalement.
                    ...
                    # Etat inattendu :
                    # intervention externe probable.
                    #
                    # On ne touche PAS au Deployment afin de ne pas écraser
                    # une modification potentiellement volontaire.

                    logger.error(
                        f"Conflit detecte avant rollback: {target} a "
                        f"{replicas_actuels} replicas, ni l'etat post-action "
                        f"({min_replicas}) ni l'etat pre-action "
                        f"({replicas_a_restaurer}). Rollback automatique annule, "
                        f"actionState reste ACTION_TAKEN."
                    )

            else:

                logger.error(
                    "Retour sous le seuil mais preActionState absent: "
                    "rollback impossible, actionState reste ACTION_TAKEN."
                )

        else:

            logger.info(
                f"OK: min_cost={valeur} <= threshold={cost_threshold}"
            )
    ...
    # -----------------------------------------------------------------------
        ...
        logger.info(
            "PAS_ASSEZ_DE_DONNEES: aucune metrique sur la fenetre, "
            "aucune action."
        )
...
def configure(settings, logger, **kwargs):
    """
    Charge la configuration Kubernetes une fois au démarrage
    de l'opérateur.
    """
    try:
        ...
        logger.info(
            "Configuration Kubernetes chargee (in-cluster)."
        )
        ...
        logger.info(
            "Configuration Kubernetes chargee (kubeconfig local)."
        )
...
def on_finops_create(
    ...
    **kwargs
    ...
    logger.info(
        f"FinOpsPolicy creee dans {meta['namespace']}, "
        f"signature initiale enregistree."
    )
...
def on_finops_update(
    ...
    **kwargs
    ...
    a_traiter, signature_actuelle = doit_traiter(
        spec,
        derniere_signature
    )
    ...
    if not a_traiter:
        ...
        logger.debug(
            "Signature inchangee (probablement notre propre patch "
            "de status) - cycle ignore."
        )
    ...
    # Toujours mettre à jour la signature en dernier, pour que le patch
    # de status déclenché par ce même appel soit ignoré au prochain passage.
...
def on_finops_timer(
    ...
    **kwargs
    ...
    """
    Source periodique d'evaluation, independante de tout changement de spec.

    C'est ce timer qui alimente concretement consecutiveExceedances
    au fil du temps - sans lui, le compteur ne progresse que sur des
    changements manuels de la FinOpsPolicy.

    Limitation documentee (YAGNI) :
    si ce timer et un vrai changement de spec se declenchent quasi
    simultanement, un double comptage de cycle est possible en theorie.
    Cas rare, pas de mecanisme de debounce ajoute pour eviter une
    complexite disproportionnee par rapport au risque.
    """
...
def on_finops_delete(
    ...
    **kwargs
    ...
    logger.info(
        f"FinOpsPolicy supprimee dans {meta['namespace']}."
    )
